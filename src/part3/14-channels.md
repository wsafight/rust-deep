...
本章要说明：**它不是隐喻** —— MIR 里就是一条 `move`，
而且这条 `move` 就是**整个并发安全论证的全部机制**。
...
不是"语义上等价于移动"，而是**真的有一条 `move` 指令**。

搬走之后 `_4` 处于**未初始化**状态 —— 借用检查器不允许再读它。
这就是 14.0 那个 E0382 的来源。

### 14.2.2 对照：`Arc` 版本是**借用**，不是 move
...
**`_10 = &_4` 是借用。** `s`（`_4`）没有被搬走，只是被借用了一下
（`Arc::clone` 内部做了一次原子加 —— 第 13 章的 `ldadd`）。
...
- 接收方**独占** `T`；
- **任一时刻只有一个线程能访问它** —— 这正是所有权规则的定义。

> **"发送即 move"不是一句宣传语，是并发安全的全部机制。**
> 编译器不需要检查"有没有别的线程在读"，它只需要检查
> **"这个值现在归谁"** —— 而后者在第 1 章就讲完了（借用检查器）。
...
**Rust 的选择是把这个运行时问题变成编译期问题**：
按值传递 → 所有权唯一 → 不需要任何运行时同步。
...
`&self` 让 `Sender` 可以共享（`Clone` 之后多生产者），
`t: T` 让消息体独占。
...
> `mpsc` 就是最好的教材 —— 它的 `unsafe` 全部在队列实现里，
...
- **MIR 里就是一条 `move`**：`_10 = move _4` ——
  不是"语义上等价于移动"，是真的有一条 `move`。
- **这条 `move` 就是并发安全的全部机制**：
  所有权唯一 → 任一时刻只有一个线程能访问 → 不需要运行时同步。
  **所有权规则免费变成了线程安全规则。**
...
- **这一章几乎没有 `unsafe`** —— 并发安全是**免费**得到的。
  而 channel 的内部实现全是 `unsafe` —— 这是"安全抽象 + 不安全实现"的样本。
