...
> 本书在没有 benchmark 数据之前不写"哪个更快"
> （见 PLAN §13 断点 6）。这里只说**结构上的差异**。
...
★ 对照 `std::sync::mpsc` 的**内部实现**：那里全是 `unsafe`
（`UnsafeCell`、原子操作、waker 的生命周期管理）。
**这就是"安全抽象 + 不安全实现"的分层**：
...
- **任务类型 `Box<dyn FnOnce() + Send + 'static>` 的三个约束各有出处**：
  `FnOnce`（第 6 章）、`Send`（第 12 章）、`'static`（线程生命周期）。
...
- **一行 `unsafe` 都没有** —— 这是"安全抽象 + 不安全实现"分层的结果：
  `mpsc` 内部全是 `unsafe`，但接口层完全安全，所以你的代码也安全。
