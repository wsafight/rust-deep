...
> 常见路线有 `Rc<RefCell<>>`、arena + 索引和裸指针；
> 当节点可由容器统一管理时，arena 往往是很稳妥的起点。

## 先把方案认清

arena 把节点集中存进 `Vec<Node>`，节点关系用稳定的 `NodeId(usize)` 表达，
而不是让节点直接持有彼此的引用。访问时由 `&Arena` 或 `&mut Arena` 统一
完成边界与借用检查，索引本身可以复制、序列化并跨较长流程保存。

直观地说：**别让节点拿着彼此的地址，给它们发门牌号。**地址会受移动和
生命周期牵制，门牌号只需要由 arena 负责解释。

### 放到业务里：AST、菜单树与工作流图

编译器 AST、权限菜单和工作流 DAG 都需要父子跳转、批量更新甚至环。
`Rc<RefCell<Node>>` 容易引入运行时借用失败和引用环；带生命周期的直接引用
...
父子关系保存的是 ID，不是指向 `Vec` 内部元素的引用；容器扩容时关系不会失效。
...
- `NodeId` 是 `Copy` 的整数 → 节点关系本身不携带 Rust 引用生命周期；
  真正读取或修改节点时，仍由对 `Arena` 的借用接受检查；
- 节点的所有者只有一个：`Arena` 的 `Vec`；
- 向上/向下访问都是"查表"，不需要所有权转移；
- 内存连续（`Vec<ArenaNode>` 是一块），缓存友好；
- 可以用小整数索引压缩节点（见 5.2.2）。

主要限制是：`NodeId` 只有和创建它的 `Arena` 放在一起才有意义。
它当然可以复制、返回或放进其他容器，但解析时必须回到原 arena；若支持删除和
槽位复用，还应加入 generation，避免旧 ID 指向一个后来入住的新节点。
...
路线 3（裸指针）可以避免某些边界检查，但并不自动比 arena 快。
它的代价是**你要自己保证**：
...
- **容器统一拥有节点时，优先考虑 arena**：`NodeId` 是 `Copy`，
  节点关系不携带生命周期；内存可连续布局，也没有强引用环。
- **`Rc<RefCell<>>` 的三笔开销**在汇编里可见：
  `drop_slow`（引用计数）、`panic_already_mutably_borrowed`（借用检查）、
  `__Unwind_Resume`（异常安全）。当前样本的静态指令数差约 4 倍，
  但这不直接等同于运行时间差 4 倍。
