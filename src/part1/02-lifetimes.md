...
生命周期参数写在泛型参数中，例如 `fn pick<'a>(x: &'a str) -> &'a str`。
它描述的是**返回引用与输入引用之间的有效范围关系**，不会延长任何值的寿命，
也不会产生运行时代码。很多常见关系可由省略规则自动补出。

最省脑子的记法是：**生命周期不是倒计时，而是引用之间的接线图。**
`'a` 不负责让数据多活一秒，只负责说明哪根线不能比它连接的对象更长。
...
这里返回值只可能来自 `cfg`，与临时查询参数 `name` 无关。精确表达这层关系，
既避免复制字符串，也防止调用方在配置已经销毁后继续持有字段引用。
