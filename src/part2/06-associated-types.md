...
但有一个间接联系：**关联类型是 `Iterator` 能被零成本优化的前提**。
因为 `Self::Item` 唯一，编译器知道 `next()` 返回什么类型，
于是能把整个迭代器链单态化 + 内联掉（第 3.5 节的向量化就是这么来的）。

如果 `Iterator` 用泛型参数，链式调用就没法这么彻底地优化——
编译器得为每个可能的 `T` 组合生成代码。

**所以"关联类型"不只是 API 设计，也是零成本抽象的**必要条件**。**
...
- **关联类型不是"更弱的泛型参数"**：`I::Item` 能出现在多个位置并自动推断，
  泛型参数版本往往要求调用者显式指定。
- **`Iterator` 用关联类型是零成本抽象的前提**：
  编译器知道 `next()` 返回什么类型，才能把整条链内联掉。
