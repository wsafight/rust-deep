...
> 它和 `fn f<'a, ...>` 的差别不是风格，是**量词的方向**——
> 前者说"对每一个 `'a` 都成立"，后者说"存在一个 `'a`"。
...
| | `weak<'a, ...>` | `strong<for<'a> ...>` |
|---|---|---|
| `'a` 是谁定的 | **调用者**（写在函数签名上） | 谁都不定——它对所有 `'a` 同时成立 |
...
| `fn f<'a, F: Fn(&'a str) -> &'a str>` | `'a` 是函数参数，**存在量词** |
