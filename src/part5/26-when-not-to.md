...
常见 unsafe 优化包括 `get_unchecked`、裸指针算术和手工构造多个可变切片。
它们省掉的是特定检查，不是笼统的“Rust 安全成本”。先查看优化后的代码和
profile，再决定是否值得承担长期的安全不变量。

把 `unsafe` 当成贷款而不是优惠券：眼前可能省下一次检查，未来每次重构都要
继续偿还那份正确性证明。若优化器已经替你消除了检查，这笔贷款就更没必要借。

### 放到业务里：解析固定格式数据包

解析器在热点循环里读取字段时，开发者常想移除边界检查。更稳妥的顺序是：
先在入口一次性验证最小包长，再使用安全切片、`chunks_exact` 或结构化解析；
...
这类写法常能让优化器消除循环内检查。若生成代码已经理想，继续改成
`get_unchecked` 只会增加维护成本。
...
★ **`unsafe` 本身不是性能开关。** 这个具体版本把引用换成裸指针后
丢失了 `noalias`，所以错过了优化；其他场景仍需分别验证。
...
在这个函数签名里，裸指针没有携带引用所提供的别名保证。

★ 这条推翻了一个很常见的直觉链：
"`unsafe` = 更底层 = 更快"。这个推理不成立。`unsafe` 关键字本身既不
增加也不删除优化信息；本例变慢的是改用裸指针后的别名信息损失。
...
4. `safe_double_add` 是 5 条指令（含 `add w8, w9, w8, lsl #1`），
   `raw_double_add` 是 8 条（多一次 `ldr w9, [x1]`）——
   **这个缺少 `noalias` 的裸指针版本更慢**。
...
- **`unsafe` 可能更慢**：裸指针不带 `noalias`，
  ...
  **`unsafe` 本身既不保证更快，也不必然更慢；要看它改变了哪些检查和优化信息。**
