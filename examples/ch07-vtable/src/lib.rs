...
pub fn dyn_area(s: &dyn Shape) -> f64 {
    s.area()
}

/// 静态分发：应被完全内联为一次乘法
pub fn static_area(s: &Sq) -> f64 {
    s.area()
}

/// ★ 构造点：强制 `Sq -> dyn Shape`，让 vtable 真正生成
#[inline(never)]
pub fn make_dyn(v: f64) -> Box<dyn Shape> {
    Box::new(Sq(v))
}
...
pub fn dyn_both(s: &dyn Shape) -> (f64, &'static str) {
    (s.area(), s.name())
}
...
/// 这是"泛型 = 单态化"的一个直接证据。
pub fn area_generic<T: Shape>(s: &T) -> f64 {
    s.area()
}

/// ★ 调用点：`area_generic::<Sq>` 会被完全内联（见汇编里没有 `bl`）
#[unsafe(no_mangle)]
pub fn call_generic(s: &Sq) -> f64 {
    area_generic(s)
}

/// 动态分发：走 vtable，应有间接调用
#[unsafe(no_mangle)]
pub fn area_dyn(s: &dyn Shape) -> f64 {
    s.area()
}
...
pub fn area_boxed(s: &Box<dyn Shape>) -> f64 {
    s.area()
}
